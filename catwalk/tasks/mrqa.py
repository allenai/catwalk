...
class MrqaTask(HFDatasetsTask):
    ...
    def dataset(self, split: str):
        ...
        ), "MRQA requires a dataset name as it contains multiple subsets"
        assert_message = (
        ...
        loaded_dataset = loaded_dataset.remove_columns(
            ["context_tokens", "question_tokens", "answers"]
        )
        ...
        def preprocess_answers(example):
            example["answers"].update(
                {
                    "answer_start": [
                        x["start"][0] for x in example["answers"]["char_spans"]
                    ]
                }
            )
        ...
        loaded_dataset = loaded_dataset.map(
            preprocess_answers, load_from_cache_file=False
        )
